...
if [ -z "$1" ]
then
  echo "Required arg, build timestamp (e.g. 20131127) not provided. Exiting"
  exit 99
fi

if [ -z "${BLD_E_PATH}" ]
then
  echo "BLD_E_PATH not set, exiting"
  exit 98
fi

if [ ! -d "${BLD_E_PATH}/$1" ]
then
  echo "Creating directory ${BLD_E_PATH}/$1"
  mkdir ${BLD_E_PATH}/$1
fi

cd ${BLD_E_PATH}/releasescripts/packages
...
#Extra Modules
#CRS./bodhi-closebuild.sh $1
#CRS./bodhi-shutdownbuild.sh $1
./cpubuild.sh $1
